...
$answers = array_map(
	function ( $text, $index ) use ( $attributes ) {
		return array(
			'index'   => $index,
			'text'    => $text,
			'correct' => $attributes['correctAnswer'] == $index,
		);
	},
	$attributes['answers'],
	array_keys( $attributes['answers'] )
);
...
>
    ...
    <ul>
		...
		foreach ( $ourContext['answers'] as $answer ) { ?>
            <li data-wp-class--no-click="callbacks.noclickclass"
                data-wp-class--fade-incorrect="callbacks.fadedclass" <?php echo wp_interactivity_data_wp_context( $answer ); ?>
				...
				<?php echo esc_html_e( $answer['text'], 'interactivity-quiz' ); ?></li>
		<?php }
		?>
    </ul>
